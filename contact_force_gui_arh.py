...
xml_path ="arh_sphere_scene1.xml"   #"left_hand_light.xml"   #"arh_sphere_scene1.xml"  #"arh_soft_scene1.xml"    #'left_hand_light_with_cube.xml'#"arh_soft_scene1.xml"   #'left_hand_light.xml' #xml file (assumes this is in the same folder as this file)
...
for x in range(simend):
  ...
  # Read the frame from the renderer
    ...
    mj.mjr_readPixels(rgb, depth, viewport1, context)
    
    # Convert RGB to BGR for OpenCV
    bgr = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
        # Save the frame as an image
    image_path = f'zzz_frame_{x:04d}.png'
    cv2.imwrite(image_path, bgr)
    # # Save the frame
    # frames.append(bgr)
    # print("frames is life")
